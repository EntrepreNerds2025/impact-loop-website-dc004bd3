/**
 * Impact Loop blog data layer.
 * Loads markdown files from /content/blog at build time via Vite's import.meta.glob,
 * parses YAML frontmatter with js-yaml, and exposes a query API.
 *
 * Mirrors the structure used on rovonnrussell.com so future blog work is consistent
...
export function getCategoryCounts(): Record<BlogCategory, number> {
  ...
  for (const post of publishedPosts) counts[post.category] = (counts[post.category] || 0) + 1;
  return counts;
}
